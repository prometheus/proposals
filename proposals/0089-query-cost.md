...
> TL;DR: A single expensive query can hurt a whole Prometheus. We have knobs to cap it (`--query.max-samples`, `--query.timeout`), but no way to tell a user *before* they run a query how expensive it is, and no per-query, reloadable ceilings. This proposal adds an approximate storage-input cost *estimate* (series touched, samples read) exposed through `/api/v1/query_cost`, reloadable cost *limits* enforced during execution, and an estimated-vs-actual `cost` object on the query response. All behind a `query-cost` feature flag.

## Why

Without this feature, Prometheus protects itself from runaway queries with startup limits:

* `--query.max-samples` caps peak samples in memory, not the total read.
...
* Per-query series and sample-read budgets are unavailable, even though duration can already be lowered with `timeout`.
...
* Give a cost *estimate* (series touched, samples read) without evaluating the expression, using bounded chunk sampling and index enumeration.
* Expose the estimate through a new API so clients can gauge cost before running a query.
* Add reloadable cost limits (`query_max_series`, `query_max_samples_read`, `query_max_duration`) enforced during execution.
...
{
  "cost": {
    "estimated": { "seriesTouched": 42, "samplesRead": 5040 },
    "actual":    { "seriesTouched": 40, "samplesRead": 4980, "peakSamples": 320 }
  }
}
```
...
3. **Reuse `--query.max-samples` and friends.** Existing startup flags cover peak samples, duration and concurrency; they do not supply series or sample-read budgets. The new series/sample settings measure different quantities, while `query_max_duration` adds a reloadable duration setting and retains the startup fallback.
